...
def get_from_dict(temp, *args):
  if str(type(temp))=="<class 'dict'>":
    if len(args)==1:
      if args[0] in temp:
        print(f"{args[0]} : {temp[args[0]]}")
      else:
        print(f"{args[0]} was not found in this dict")
    else:
      print("You need to send a word to search for")
  else:
    print(f"You need to send a dictionary. You send: {type(temp)}")
  pass

def update_word(temp, *args):
  if str(type(temp))=="<class 'dict'>":
    if len(args)==2:
      if args[0] in temp:
        temp[args[0]] = args[1]
...
def delete_from_dict(temp, *args):
  if str(type(temp))=="<class 'dict'>":
    if len(args)==1:
      if args[0] in temp:
        del temp[args[0]]
